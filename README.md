...
* Check the `ConnectionStrings` in `appsettings.json` files under the `DinheiroSobControle.Blazor` and `DinheiroSobControle.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 242b0b30-937e-4d63-a2ff-979c51ce86c0
```

> `242b0b30-937e-4d63-a2ff-979c51ce86c0` is the password of the certificate, you can change it to any password you want.
...
Run `DinheiroSobControle.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `DinheiroSobControle.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `DinheiroSobControle.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
